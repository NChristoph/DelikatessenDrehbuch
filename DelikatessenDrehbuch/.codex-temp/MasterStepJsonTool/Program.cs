...
var path = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "wwwroot", "data", "master_steps.json"));
var root = JObject.Parse(File.ReadAllText(path));
var meta = (JObject)root["_meta"]!;

meta["version"] = "2.0.0";
meta["updated_at"] = "2026-03-12";
meta["supported_languages"] = new JArray("de", "en", "esp", "prt", "id", "nl", "sv", "da", "no", "ms");
meta["stable_key_policy"] = JObject.Parse(@"{
  ...
  'no': 'Ny metadata bruker bare stabile semantiske nøkler som category_keys eller ingredient_family_keys. Ingen database-ID-er og ingen frie brukerinput lagres som permanente referanser.',
  'ms': 'Metadata baharu hanya menggunakan kunci semantik yang stabil seperti category_keys atau ingredient_family_keys. Tiada ID pangkalan data dan tiada input bebas pengguna disimpan sebagai rujukan kekal.'
}");
...
root["variable_type_definitions"] = JObject.Parse(@"{
...
root["step_intent_definitions"] = JObject.Parse(@"{
...
root["category_taxonomy"] = JObject.Parse(@"{
...
root["diet_taxonomy"] = JObject.Parse(@"{
...
root["ingredient_family_definitions"] = JObject.Parse(@"{
...
root["quality_rule_definitions"] = JObject.Parse(@"{
...
foreach (var step in ((JArray)root["master_steps"]!).OfType<JObject>())
{
    var variables = step["variables"] is JArray varArray ? varArray.Values<string>().ToList() : new List<string>();
    var aliases = step["aliases"] is JArray aliasArray ? aliasArray.Values<string>().ToList() : new List<string>();
    var action = step.Value<string>("action") ?? string.Empty;
    var phase = step.Value<int?>("phase") ?? 0;
    ...
    {
        types[name] = variableTypeMap.TryGetValue(name, out var mapped) ? mapped : "free_text";
    }
    step["variable_types"] = types;

    var intent = GetStepIntent(action, phase);
    step["step_intent"] = intent;
...
{
    if (!array.Values<string>().Contains(value, StringComparer.Ordinal))
    {
        array.Add(value);
    }
}
...
static string GetStepIntent(string action, int phase)
{
    ...
    {
        "wash" or "peel" => "prep_base",
        "cut" or "grate" or "mince" => "prep_cut",
        "marinate" or "soak" or "mix_dry" or "knead" or "dough_rise" or "portion" or "roll_out" or "bread_3step" or "whisk" or "fold_in" or "layer" => "prep_mix",
        "heat" or "saute" or "fry" or "deepfry" or "bake" or "roast" or "grill" or "stir" or "add" or "gratinate" => "cook_heat",
        "boil" or "simmer" or "steam" or "deglaze" or "reduce" or "thicken" or "drain" or "strain" or "cover" or "braise" => "cook_liquid",
        "blend" or "season_adjust" => "finish_season",
        "cool" or "serve" => "finish_serve",
        _ when phase <= 1 => "prep_base",
        _ when phase == 2 => "cook_heat",
        _ when phase == 3 => "finish_season",
        _ => "finish_serve"
    };
}
...
static int GetEstimatedClipSeconds(string action)
{
    ...
    {
        "wash" => 4,
        "cut" or "grate" or "mince" or "boil" or "simmer" or "bake" or "roast" or "grill" or "serve" => 5,
        "mix_dry" or "saute" or "fry" or "deepfry" or "blend" => 6,
        "knead" => 7,
        "dough_rise" or "season_adjust" => 4,
        _ => 5
    };
}
