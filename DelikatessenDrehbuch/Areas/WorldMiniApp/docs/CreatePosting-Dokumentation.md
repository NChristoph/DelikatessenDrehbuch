...
â”‚                     CreatePosting.cshtml                      â”‚
â”‚  â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â” â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â” â”‚
â”‚  â”‚ Basics  â”‚ â”‚ Media  â”‚ â”‚Zutaten   â”‚ â”‚Steps â”‚ â”‚Keywords  â”‚ â”‚
â”‚  â”‚card-    â”‚ â”‚card-   â”‚ â”‚card-     â”‚ â”‚card- â”‚ â”‚card-     â”‚ â”‚
â”‚  â”‚basics   â”‚ â”‚media   â”‚ â”‚ingredien.â”‚ â”‚steps â”‚ â”‚keywords  â”‚ â”‚
...
Definiert in `CreatePosting.cshtml` (Zeilen 2679â€“2695):

```
1.  create-posting-utils.js          â† sofort (kein defer)
2.  create-posting-data-urls.js      â† sofort
3.  create-posting-data-store.js     â† sofort
4.  create-posting-feedback.js       â† sofort
5.  create-posting-page-data.js      â† sofort
6.  create-posting-template-builder.js â† sofort
7.  CreatePostingSmartStepCreator.js â† async/defer
8.  master-step-renderer.js         â† async/defer
9.  recipe-step-suggest.js          â† async/defer
10. create-posting-probability.js   â† async/defer
11. CreatePostingPage.js            â† async/defer
12. create-posting-publish-checklist.js â† async/defer
```
...
window.CreatePostingDataUrls       â† URL-Registry
    â†“
window.CreatePostingDataStore      â† Nutzt DataUrls + Utils
    â†“
window.CreatePostingFeedback       â† Nutzt Utils
    â†“
window.CreatePostingPageData       â† Nutzt DataStore
    â†“
window.CreatePostingTemplateBuilder
    â”œâ”€â”€ Nutzt: MasterStepRenderer
    â”œâ”€â”€ Nutzt: getThemeMutedTextClass()
    â””â”€â”€ Nutzt: getCreatePostingTheme()
    â†“
window.MasterStepCreatorHelpers    (aus CreatePostingSmartStepCreator.js)
    â”œâ”€â”€ Nutzt: CreatePostingUtils
    â”œâ”€â”€ Nutzt: CreatePostingDataStore
    â”œâ”€â”€ Nutzt: CreatePostingIngredientHelpers
    â””â”€â”€ Setzt: ingredientTransforms, stepCatalog
    â†“
window.MasterStepRenderer
    â”œâ”€â”€ Nutzt: CreatePostingDataStore
    â””â”€â”€ Nutzt: CreatePostingUtils
    â†“
window.RecipeStepSuggest
    â”œâ”€â”€ Nutzt: MasterStepRenderer
    â””â”€â”€ Nutzt: CreatePostingDataStore
    â†“
window.CreatePostingProbability
    â”œâ”€â”€ Nutzt: RecipeStepSuggest
    â”œâ”€â”€ Nutzt: MasterStepRenderer
    â””â”€â”€ Nutzt: CreatePostingIngredientHelpers
    â†“
CreatePostingPage.js (Haupt-Einstiegspunkt)
    â”œâ”€â”€ Nutzt: ALLE oben genannten
    â”œâ”€â”€ Setzt: CreatePostingIngredientHelpers
...
#### Form-Felder (Form â†’ Server Binding)

**Zutaten:**
```
IngredientMeasureQuantity[i].IngredientId    â†’ int
IngredientMeasureQuantity[i].MeasureId       â†’ int
IngredientMeasureQuantity[i].Quantity         â†’ decimal
```

**Steps:**
```
RecipePreperationSteps[i].PreperationStepId  â†’ int
RecipePreperationSteps[i].StepIndex          â†’ int
RecipePreperationSteps[i].RecipePreperationStep.Step_DE  â†’ string
RecipePreperationSteps[i].RecipePreperationStep.Step_EN  â†’ string
RecipePreperationSteps[i].RecipePreperationStep.Step_ESP â†’ string
RecipePreperationSteps[i].RecipePreperationStep.Step_PRT â†’ string
RecipePreperationSteps[i].RecipePreperationStep.Phase    â†’ int
RecipePreperationSteps[i].RecipePreperationStep.Equipment â†’ int
```

**Smart Steps:**
```
SmartStepReferences[i].MasterStepKey   â†’ string
SmartStepReferences[i].MetadataJson    â†’ string (JSON)
SmartStepReferences[i].StepIndex       â†’ int
```

**Keywords:**
```
SelectedKeywordIds[i] â†’ int
```
...
<div class="creator-bg min-vh-100">
  ...
  <form asp-action="UploadNewVideo" method="post" id="recipeForm"
    ...
    <!-- â•â•â•â•â•â• SECTION 1: Basics (card-basics) â•â•â•â•â•â• -->
    ...
    <!-- â•â•â•â•â•â• SECTION 2: Media (card-media) â•â•â•â•â•â• -->
    ...
    <!-- â•â•â•â•â•â• SECTION 3: Zutaten (card-ingredients) â•â•â•â•â•â• -->
...
### _SmartStepCreatorPartial.cshtml â€” HTML-Bereich (Zeilen 479â€“519)
...
Definiert in `_SmartStepCreatorPartial.cshtml` (Zeilen 1â€“477).
...
| `.ingredient-chip.ingredient-chip-dimmed` | 313 | Opacity 0.4, grayscale(0.5), order 99 â€” nicht-passende Zutaten bei gefilterter Anzeige |
...
| Klasse | Zeile | Eigenschaft |
|--------|-------|-------------|
| `.fraction-picker-row` | 311 | âš ï¸ **DEPRECATED** - Border-top 1px solid, padding-top 8px, margin-top 6px |
| `.fraction-pick` | 316 | âš ï¸ **DEPRECATED** - Font-size 0.78rem, padding 4px 10px, border-radius 12px |
| `.fraction-pick.active` | 323 | âš ï¸ **DEPRECATED** - Lila border/shadow/background |
...
**Optionale Segmente:** `[optional text {{variable}}]` â€” wird ausgeblendet wenn Variable leer
  ...
  "special_transforms": [
    {
      ...
      "outputs": [{ "names": { "de": "Beurre noisette" }, "icon": "ðŸ§ˆ" }]
    }
  ]
}
```
...
<div class="step-filter-controls" data-theme="...">
  <div class="step-search-wrapper">
    <input class="step-search-input" placeholder="ðŸ” Step suchen...">
  </div>
  <div class="phase-tabs">
    <button class="phase-tab active" data-phase="all">Alle</button>
    <button class="phase-tab" data-phase="1">ðŸ”ª Vorbereitung</button>
    <button class="phase-tab" data-phase="2">ðŸ”¥ Kochen</button>
    <button class="phase-tab" data-phase="3">âœ¨ Finishing</button>
    <button class="phase-tab" data-phase="4">ðŸ½ï¸ Servieren</button>
  </div>
</div>
```

**Event-Flow:**
1. User tippt in `.step-search-input` â†’ `input` Event
...
**Pfad:** `wwwroot/js/create-posting-probability.js`
...
const probabilityDraft = draftEngine?.getProbabilityDraft(masterId)  // â† Draft-Engine (falls vorhanden)
                      || window.probabilityStates?.[masterId];       // â† Unified System (Fallback)
...
helpers.renderEditableStepPreview(draft, {
    ...
    actionHtml  // Accept/Dismiss Buttons
})
```
...
#### Fraction-Helpers (Zeile 95â€“189)
...
**Stand 2026-04-03:** Gemeinsamer Editor-Workflow für beide Bereiche.

**Architektur:**
```javascript
// 1. Gemeinsame Draft-State-Verwaltung
activeStepDraft = { master_id, templateRaw, values, _multiIngredients }
probabilityDrafts[masterId] = { masterId, templateRaw, values, _multiIngredients }

// 2. Einheitlicher Editor-Einstieg
openStepDraftEditor({
  source: 'manual' | 'suggested',
  varName, currentVal, masterId,
  context: { type: 'step' | 'probability', source, masterId, ... },
  onApply, onClose
})
...
updateContextValue(context, varName, value, extras) {
  if (context.type === 'step') {
    activeStepDraft.values[varName] = value;
    renderMasterText();
  } else if (context.type === 'probability') {
    probabilityDrafts[masterId].values[varName] = value;
    renderProbabilityTemplate(masterId);
  }
}

// 4. Overlay-Header rendert Preview direkt aus Draft + Master-Step-Vorlage
renderOverlayPreviewForContext(context)
```

**Vorteile:**
- Gleiches Overlay für oberen und unteren Bereich
- Vorschläge oben und manuelle Auswahl unten teilen denselben Draft-State
- Preview im Overlay basiert auf Master-Step-JSON statt auf DOM-Kopien
- Multi-Ingredient-Workflow funktioniert identisch in beiden Bereichen
- `Step akzeptieren` sitzt im Overlay-Header für beide Modi

**Implementierung:**

| Funktion | Ort | Beschreibung |
|----------|-----|-------------|
| `openStepDraftEditor(config)` | `CreatePostingSmartStepCreator.js` | Gemeinsamer Einstieg für beide Modi. Normalisiert `source` und delegiert an `openUniversalVariableEditor(config)` |
| `openUniversalVariableEditor(config)` | `CreatePostingSmartStepCreator.js` | Baut das Fullscreen-Overlay für beide Bereiche |
| `renderOverlayPreviewForContext(context)` | `CreatePostingSmartStepCreator.js` | Rendert den Header-Preview direkt aus Draft + Master-Step-Vorlage |
| `createUnifiedOverlayHtml(editorHtml, context)` | `CreatePostingSmartStepCreator.js` | 3-Section Layout Builder: Header-Preview, Editor-Inhalt, Footer-Buttons |
| `bindUnifiedOverlayEventHandlers(...)` | `CreatePostingSmartStepCreator.js` | Gemeinsame Event-Bindings inkl. `BtnAcceptProbabilityStep` und `BtnAcceptStepOverlay` |
| `updateContextValue(context, varName, value, extras)` | `CreatePostingSmartStepCreator.js` | Aktualisiert Step- oder Probability-Draft und rendert den Preview neu |
| `saveCurrentEditorValueBeforeAccept(contextType, contextId)` | `CreatePostingSmartStepCreator.js` | Gemeinsamer Accept-Helper für beide Modi |
| `buildProbabilityEditorConfig(...)` | `CreatePostingPage.js` | Baut den `suggested`-Editor-Kontext für den oberen Bereich |
| `acceptProbabilityTemplateStep(masterId, probabilityVars)` | `CreatePostingPage.js` | Übernimmt einen vorgeschlagenen Draft in die Step-Liste unten |

**Verwendung in beiden Bereichen:**

```javascript
// Unten: freier manueller Einstieg
openStepDraftEditor({
  source: 'manual',
  context: { type: 'step', source: 'manual', tokenId }
});

// Oben: vorgeschlagener Einstieg
openStepDraftEditor({
  source: 'suggested',
  context: { type: 'probability', source: 'suggested', probabilityMasterId: masterId }
});
```
...
| Funktion | Zeile | Beschreibung |
|----------|-------|-------------|
| `acceptActiveStep()` | 1706 | Aktiven Step akzeptieren â†’ `window.addStep()` aufrufen |
...
#### Exportierte Probability-Helpers (Zeile 2019â€“2180, 2819â€“2837)
...
### Zutat-Konfiguration (Zeile 261â€“327)
...
|-------|----------|-------|-------------|
| click | `.probability-template-wrap .template-var, .js-optional-var-add, .js-probability-var` | current | Oeffnet den gemeinsamen Editor ueber `openStepDraftEditor({ source: 'suggested' })` |
| click | `.probability-template-wrap .ingredient-plus-btn` | current | Oeffnet denselben gemeinsamen Overlay-Pfad fuer Multi-Ingredient-Bearbeitung |
| click | `#BtnAcceptProbabilityStep` | current | Akzeptiert den vorgeschlagenen Draft direkt aus dem Overlay-Header |
| click | `button[data-pick-mode]` (in `#universalEditorOverlay`) | current | Shared Pick-Mode-Handler aus `CreatePostingSmartStepCreator.js` |
...
**Wichtig nach dem Refactor:**
- Kein `prob-inline-editor-host` mehr
- Kein separates Probability-Editorsystem mehr
- Kein `window.ProbabilityMultiIngredients` mehr
- Plus/Remove/Apply/Accept laufen ueber denselben Overlay-State wie unten

### Zutaten-Laden & Grammatik (Zeile 654â€“764)
...
### Spezial-Editoren (Zeile 1657â€“2530)
...
| Pronoun | `openPronounEditorForToken()` | 1823 | â€“ |
...
### Artikel-Auswahl-System (Zeile 1847â€“2060)
...
| Funktion | Zeile | Beschreibung |
|----------|-------|-------------|
| `normalizeDecimalInputValue(input)` | 3047 | Dezimal normalisieren (,â†’.) |
...
### Zutat-Verwaltung (Zeile 3112â€“3388)
...
### Keyword-Verwaltung (Zeile 3499â€“3530)

| Funktion | Zeile | Beschreibung |
|----------|-------|-------------|
| `toggleKeyword(id, btn)` | 3499 | Keyword-Auswahl umschalten |

### Window-Exports (Zeile 2963â€“2970)
...
|----|---------|-----------|-------------|
| `recipeForm` | `<form>` | â€“ | Haupt-Formular |
...
User öffnet CreatePosting
    ...
    â”‚   â”‚   â”œâ†’ applyDerivedIngredientRowVisuals()
    â”‚   â”‚   â””â†’ refreshIngredientProbabilityHints()
    â”‚   â”‚       â””â†’ RecipeStepSuggest.detectRecipeTypes()
    â”‚   â”‚           â””â†’ #ingredientProbabilityBadges aktualisieren
    â”‚   â””â†’ scheduleCreatePostingDraftSave()
    â”‚
    â”œâ”€4. Steps erstellen (Smart Step Creator)
    â”‚   â”œâ†’ Template-Karte klicken â†’ onStepCardClick()
    â”‚   â”‚   â””â†’ renderMasterText() (Preview mit Platzhaltern)
    â”‚   â”‚
    ...
    â”‚   â”‚
    â”‚   â”œâ†’ Wert einsetzen â†’ applyCurrentEditorSelection()
    â”‚   â”‚   â”œâ†’ Fraction-aware: composeFractionText()
    â”‚   â”‚   â””â†’ activeStep.values[varName] = composed
    â”‚   â”‚       â””â†’ rerenderAfterValueSet() â†’ renderMasterText()
    â”‚   â”‚
    â”‚   â””â†’ Step akzeptieren â†’ acceptActiveStep()
    â”‚       â””â†’ window.addStep(id, null, text, {
...
Step 8: User klickt X-Button auf "Zucker"-Chip
    â†’ Remove-Handler entfernt Index 0 aus _multiIngredients
    â†’ Liste wird neu formatiert: "ein Drittel der Butter"
...
- **Step Creator:** aktiver Step-Draft
- **Probability Area:** gemeinsamer Probability-Draft pro `masterId`
...
2. â†’ getSelectedIngredientsFromPage()
   â†’ alle Zutaten mit isLiquid, isFat, isHard, isSoft, groupId
3. â†’ filterIngredientsByVarType(items, varName)
   â†’ Mapping:
...
6. Wenn KEINE passende Zutat gefunden â†’ Fallback: alle Zutaten normal anzeigen

Datenquelle (data-* Attribute auf .ingredient-db-row):
  data-is-liquid="true/false"   â† DB: is_liquid (bool)
  data-is-fat="true/false"      â† DB: is_fat (bool)
  data-is-hard="true/false"     â† DB: is_hard (bool)
...
2. DOMContentLoaded â†’ Async-Scripts starten
   ...
   â”‚   â”œâ†’ loadJson() â†’ master_steps.json
   â”‚   â”œâ†’ loadVariableCatalog() â†’ master_step_variables.json
   â”‚   â”œâ†’ renderStepButtons()
   â”‚   â”œâ†’ renderMasterText()
   â”‚   â””â†’ wireEvents()
   â”‚
   â”œâ†’ master-step-renderer.js: load()
   â”‚   â”œâ†’ master_steps.json
   â”‚   â”œâ†’ master_step_option_rules.json
   â”‚   â”œâ†’ recipe_type_step_variables.json
   â”‚   â””â†’ recipe_step_mapping.json
   â”‚
   â”œâ†’ recipe-step-suggest.js: loadMapping()
   â”‚   â”œâ†’ recipe_step_mapping.json
   â”‚   â””â†’ recipe_category_scoring.json
   â”‚
   â”œâ†’ ingredientmanager.js: refreshUnits()
   â”‚
   â””â†’ create-posting-publish-checklist.js: initChecklist()

3. jQuery Ready â†’ CreatePostingPage.js
   â”œâ†’ Theme initialisieren (readStoredCreatePostingTheme)
   â”œâ†’ loadIngredientArticleRules()
   â”œâ†’ loadIngredientTransforms()
   â”œâ†’ Alle Event-Listener binden
   â”œâ†’ showDraftRestoreBannerIfNeeded()
   â”‚   â”œâ†’ consumeCreatePostingDraftResetFlag()
   â”‚   â”‚   â””â†’ Falls Flag: clearCreatePostingDraft()
   â”‚   â”œâ†’ hasMeaningfulCreatePostingDraft()
   â”‚   â”‚   â””â†’ Falls ja: Banner #draftRestoreBanner einblenden
   â”‚   â””â†’ User klickt "Wiederherstellen" â†’ restoreCreatePostingDraft()
   â”‚       oder "Verwerfen" â†’ clearCreatePostingDraft()
   â””â†’ updateLanguageLabels()
```
...
### 2026-04-02 â€” Bugfixes: Multi-Ingredient-System & Artikel-Grammatik

#### Kritische Bugfixes

**1. CreatePostingPage.js â€” `localizeIngredientValueForSandbox()` (Zeile 1030-1059):**
...
**Transformierte Zutaten â€” Adjektiv-Deklination:**
...
- `localizeIngredientValueForSandbox()` (Zeile 1030-1059) â€” Early-Return bei gleicher Sprache
- `buildVariablesForTemplate()` (Zeile 2604-2614) â€” Optionale Variablen leer lassen statt Key-Name
...
- Chip-Click-Handler (Zeile 2148-2159) â€” Auto-Artikel via genusToArticle()
...
**CreatePostingSmartStepCreator.js:**
- `filterIngredientsByVarType()` â€” Filter erweitert (hard, soft)
...
View: data-is-fat="true/false" (HTML data-attribute)
  â†“
JS: getBaseSandboxIngredients() â†’ isFat: boolean
  â†“
...
rerenderAfterValueSet()
  â†’ Template wird neu gerendert mit aktualisierten Werten
```
...
| **DA** | {{action}} {{ingredient}} grundigt under koldt vand[, fjern {{removal}}] og dup {{pronoun}} tÃ¸r. |
| **NO** | {{action}} {{ingredient}} grundig under kaldt vann[, fjern {{removal}}] og tork {{pronoun}} tÃ¸rr. |
...
```
âŒ `.prob-inline-editor` fehlt komplett
...
- Ohne diese Klasse konnte der Editor nicht gefunden werden â†’ "Editor element not found in overlay!"
...
- Ei-Aufteilung (Ei â†’ Eiklar/Eigelb) wird korrekt aktualisiert
