...
// ─── Ingredient Card ──────────────────────────────────────────
function RDIngredientCard({ item, scale = 1 }) {
...
// ─── Body: Ingredients ────────────────────────────────────────
function RDIngredientsBody({ servings, setServings }) {
        ...
        {RD_INGREDIENT_GROUPS.map(g => (
          <div key={g.label}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {g.items.map(item => <RDIngredientCard key={item.name} item={item} scale={scale} />)}
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="mp-screen" style={{ background: '#f5efe1', position: 'relative' }}>
      <div style={{ flex: 1, overflowY: 'auto' }} className="mp-feed">
        ...
        {tab === 'ingredients' && <RDIngredientsBody servings={servings} setServings={setServings} />}
